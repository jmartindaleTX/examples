...
def chart():
    ...
    df=exec_sql(curr_sess,' select     any_value(start_station) from_station_name, any_value(end_station) end_station_name, start_lat, start_lon, end_lat, end_lon,  count(*) num_trips,           avg(datediff(minute, starttime, stoptime))::integer avg_duration_mins           from citibike.demo.trips_stations_weather_vw       where         (start_lat is not null and end_lat is not null)            and start_lat <> end_lat            group by  start_lat, start_lon, end_lat, end_lon            order by num_trips  desc         limit 20;')    
...
def rolechart()    :
            ...
            write_env(curr_sess)
            sqlstr='SELECT enabled_roles.role_name child, applicable_roles.grantee parent  FROM snowflake.information_schema.enabled_roles JOIN snowflake.information_schema.applicable_roles ON enabled_roles.role_name = applicable_roles.role_name'
...
def timetravel():
    ...
    curr_table = "CITIBIKE.DEMO.TRIPS"
